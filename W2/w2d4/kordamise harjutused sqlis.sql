...
select s.product_id, round(sum(s.sales_sum),2) 
from salestable s
...
select s.customer_id,  round(sum(s.sales_sum),2)
from salestable s 
...
select s.sales_rep_id , round(sum(s.sales_sum),2) 
from salestable s 
...
select to_char(sale_date,'YYYY') as year, SUM(s.sales_sum )  --to char teeb kuupäeva tekstiks --variant 3. select left(sale_date,4) as sale_year, sum (sales_sum)
from salestable as s                                                                         
group by year ---variant 2. group by to_char(sale_date,'YYYY')                                ---3. left(sale_date,4)
order by year asc; --2.order by TO_CHAR (sale_date, 'YYYY')asc
...
select s.sales_sum,
 CASE 
    ...
    ELSE 'Large Sale' ---võib lisada ka oher ntx kui on tühimüük, et siis teame, et läheb otheri alla
...
(SELECT s.quantity, s.sales_sum,
	 CASE 
		when s.sales_sum < 250 THEN 'Small Sale'
  		WHEN s.sales_sum >= 250 and s.sales_sum  <= 500 then 'Medium Sale' --!!! ma ei saa aru, miks üleval olevaid qutityid ei tule alla 
   		ELSE 'Large Sale'
        END AS sales_category			
      	count(*) as number_of_sales,
	    round(sum(sales_sum)::numeric,0) as sales_sum  --:: tähendab, et võta selle formaadina
from salestable as s)
select  sales_category as kategooriad, sum(quantity) as kogus, sum(sales_sum) as summa
from myygisummakategooriad 
group by sales_category, s.quantity
;
